/// @description Insert description here
// You can write your code in this editor
//sanddollar
draw_sprite(spr_hud_sanddollar, 0, sanddollar_icon_pos[0], sanddollar_icon_pos[1]);
draw_text_transformed(sanddollar_text_pos[0], sanddollar_text_pos[1], sanddollars, 1.25, 1.25, 0);

//air
draw_sprite(spr_hud_tank, 0, air_level_icon_pos[0], air_level_icon_pos[1]);
draw_text_transformed(air_level_text_pos[0], air_level_text_pos[1], air_level, 1.25, 1.25, 0);

//air level
draw_sprite_part(spr_hud_tank_bubbles, anim_timer/10, 0, air_bubble_start, sprite_get_width(spr_hud_tank_bubbles), air_bubble_height, air_level_icon_pos[0] + 10 + 8, air_level_icon_pos[1] + 10 + 2 + air_bubble_start);